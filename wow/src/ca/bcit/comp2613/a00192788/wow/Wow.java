...
public class Wow {
	...
	public static void main(String[] args) {
		PropertyConfigurator.configure(
				Wow.class.getResourceAsStream("log4j.properties")
		);
		Logger log = Logger.getLogger(Wow.class);
		...
		log.info("This is an the full character list");
		printCharacters(characters);
		checkAndRemoveAnyHordeSpies(characters);
		log.info("This is the list without Orcs.");
		printCharacters(characters);
		try {
			checkForPandaren(characters);
					}
		catch (DontHaveTheExpansionException e) {
			log.error("ERROR", new Exception("DontHaveTheExpansionException"));
			System.out.println("\nYou need to buy the expansion pack.  Now!!");
		}
		finally {
			System.out.println("\nThe missing level is " + getLevelOfMissingCharacter());
		}
	}
	...
	private static int getRandomLevel() {
		return rand.nextInt(85)+1; // * returns a number between 1 and 85
	}
	...
	private static Comparator<Character> createCharacterComparator() {
		...
		return new Comparator<Character>() {
			...
			public int compare(Character character1, Character character2) {
				// compares Characters by level, strength and armour
				// * found and fixed the bug
	...
	private static void checkAndRemoveAnyHordeSpies(ArrayList<Character> characters) {
		...
		while (iterator.hasNext()) {
			...
			if (character.getRace() == Race.ORC) { // * fixed
				iterator.remove();
			}
		}
	}
...
//		}
	...
	private static void checkForPandaren(ArrayList<Character> characters) throws DontHaveTheExpansionException {
		// if there are any PANDAREN races in our List, throw a DontHaveTheExpansionException
		for (Character character : characters) {
	...
	// implemented getLevelOfMissingCharacter
	public static int getLevelOfMissingCharacter() {
		...
		// note: solution based on criteria that there are no duplicates and only
		// one level is missing
		
		int completeSum = (85 * (85 + 1)) / 2;  /* sum of all numbers between 1 and 85 inclusive */
		int missingSum = 0;  /* sum of all levels represented in array */
		
		for (Character character : characters) {
			missingSum = missingSum + character.getLevel();
		}
